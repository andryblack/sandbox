...
namespace Sandbox {
    ...
    SoundInstancePtr    Sound::PlayEx(float fadeIn,float vol,float pan) {
        if (!m_effect) return SoundInstancePtr(new SoundInstance(shared_from_this(),0,0.0f));
        ...
        SoundInstancePtr res = SoundInstancePtr(new SoundInstance(shared_from_this(),instance,initialVol));
        if (fadeIn!=0.0f) {
