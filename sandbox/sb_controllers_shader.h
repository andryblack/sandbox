...
namespace Sandbox {
	...
	namespace Impl {
		...
		struct LinearControllerSetUniformFloat {
			static void SetValue( const sb::intrusive_ptr<ShaderFloatUniform>& tr,float s) {
				tr->SetValue(s);
			}
		};
	}
	
	typedef LinearController<float,Impl::LinearControllerSetUniformFloat,sb::intrusive_ptr<ShaderFloatUniform> > ControllerShaderFloatUniform;
}
